...
def write_data_file(node, date_time, value):
    try:
        with mysql.connector.connect(
            ...
            insert_query = (
                "INSERT INTO sensor_data (node, date_time, value) VALUES (%s, %s, %s)"
            )
            with connection.cursor() as cursor:
...
def serial_thread():
    ...
    while running is True:
        ...
        while buffer:
            ...
            write_data_file(source_address_64, date_time_str, data_ascii)
...
def data():
    ...
    print(
        "Received Request - Range:", range_param, "Device:", device_param
    ...
    if device_param:
        try:
            with mysql.connector.connect(
                ...
                filtered_data = [
                    {"date_time": row[0].timestamp() * 1000, "value": row[1]}
                    for row in data
                ]
